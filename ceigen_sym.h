 ...
 * Author: veal
 *
 * Created on March 17, 2011, 7:39 AM
 */
#include <libio.h>
...
void eigsrt(NRvector<Comp> &d, NRmatrix<Comp> *v=NULL)
{
	Int k;
	Int n=d.size();
	for (Int i=0;i<n-1;i++) {
...
struct Symmeig {
	...
	Symmeig(const NRmatrix<Comp> &a, Bool yesvec=true) : n(a.nrows()), z(a), d(n),
		e(n), yesvecs(yesvec)
	{
		tred2();
		tqli();
	...
	Symmeig(const NRvector<Comp> &dd, const NRvector<Comp> &ee, Bool yesvec=true) :
